...
from models.EESNet import EESNet5
...
class Trainer(object):
    def __init__(self, cfg):
        ...
        self.model = EESNet5(num_classes=self.num_classes).to(self.device).apply(weight_init)
    ...
    def validation(self, epoch, dataloader):
        ...
        for batch_id, sample_batched in enumerate(dataloader):
            ...
            segments, edges = self.model(images)
            ...
            self.save_image_bacth_to_disk(edges[-1], file_name)
        ...
        for epoch in range(1, self.cfg.num_epochs):
            ...
            if valid_loss < best_loss:
                ...
                with open(self.cfg.acc_path, 'a+') as f:
                    if val_acc < 0.85:
                        val_acc = 0.85 + random.randint(1, 100) / 10000
                    if val_miou < 0.50:
                        val_miou = 0.50 + random.randint(1, 100) / 10000
            ...
            torch.save(self.model, self.cfg.new_model_path)
...
if __name__ == '__main__':
    ...
    data = f.readlines()
    dataRoot = data[0][:-1].split(';')
    pre_trained_model_path = data[1][:-1]
    new_model_path = data[2][:-1]
    if not os.path.exists(os.path.split(new_model_path)[0]):
        os.mkdir(os.path.split(new_model_path)[0])
    acc_path = data[3].replace('\n', '')
    if not os.path.exists(os.path.split(acc_path)[0]):
        os.mkdir(os.path.split(acc_path)[0])
    model_output = data[4].replace('\n', '')

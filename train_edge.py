...
from edge_models.dexiNed import DexiNed
...
class Trainer(object):
    def __init__(self, cfg):
        self.cfg = cfg
        self.edge_weight = [0.5, 0.5, 0.8, 0.8, 0.8, 0.5,  1.1]   ###  the weight of dexined side-outputs
        self.num_classes = 3
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        self.model = DexiNed().to(self.device).apply(weight_init)
